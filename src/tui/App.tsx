...
export const App: React.FC = () => {
    ...
    const handleInstallSelection = useCallback((command: string, displayName: string, useBunx: boolean) => {
        void getExistingStatusLine().then((existing) => {
            ...
            if (existing && !isAlreadyInstalled) {
                ...
                message = `cc-hud is already installed in ${getClaudeSettingsPath()}\nUpdate it with ${command}?`;
            } else {
                message = `This will modify ${getClaudeSettingsPath()} to add cc-hud with ${displayName}.\nContinue?`;
            }
    ...
    const handleInstallUninstall = () => {
        if (isClaudeInstalled) {
            // Uninstall
            setConfirmDialog({
                message: `This will remove cc-hud from ${getClaudeSettingsPath()}. Continue?`,
                action: async () => {
