...
import { getActivityMetrics } from './utils/activity-parser';
...
async function renderMultipleLines(data: StatusJSON) {
    ...
    // Check if activity widgets are needed
    const activityWidgetTypes = ['tools-activity', 'agents-activity', 'todos-progress', 'usage-limits'];
    const hasActivityWidgets = lines.some(line => line.some(item => activityWidgetTypes.includes(item.type)));
    ...
    let activityMetrics: ActivityMetrics | null = null;
    if (hasActivityWidgets && data.transcript_path) {
        activityMetrics = await getActivityMetrics(data.transcript_path);
    }
